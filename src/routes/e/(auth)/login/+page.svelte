...
<div class="w-full max-w-sm">
	<div class="text-center">
		<h1 class="font-display text-2xl font-semibold text-text-primary">Welcome back</h1>
	...
	<form class="mt-8 space-y-4" onsubmit={(e) => { e.preventDefault(); }}>
		...
		<div>
			...
			<div class="relative mt-1.5">
				...
				<input id="password" type={showPassword ? 'text' : 'password'} bind:value={password} placeholder="Enter your password" required class="w-full rounded-full border border-border bg-white/80 py-2.5 pl-10 pr-10 text-sm text-text-primary placeholder:text-text-secondary/60 focus:border-accent-primary focus:outline-none focus:ring-2 focus:ring-accent-primary/20" />
				<button type="button" onclick={() => (showPassword = !showPassword)} class="absolute right-3 top-1/2 -translate-y-1/2 text-text-secondary hover:text-text-primary" aria-label="Toggle password visibility">
		...
		<div class="flex items-center justify-between">
			<label class="flex items-center gap-2">
				<input type="checkbox" bind:checked={rememberMe} class="h-4 w-4 rounded border-border text-accent-primary focus:ring-accent-primary" />
				<span class="text-sm text-text-secondary">Remember me</span>
			</label>
			<a href={p('/forgot-password')} class="text-sm font-medium text-accent-primary hover:underline">Forgot password?</a>
		</div>

		<button type="submit" class="glow w-full rounded-full bg-accent-primary py-2.5 text-sm font-medium text-white">Sign In</button>
	</form>
	...
	<p class="mt-8 text-center text-sm text-text-secondary">
		...
		<a href={p('/register')} class="font-medium text-accent-primary hover:underline">Create one</a>
	</p>
</div>
